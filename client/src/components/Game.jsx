...
class Game extends React.Component {
  ...
  //handle players waiting for another player to connect to their room
  handleLoading() {
    lobbyCheck(b => {
      if (b === true) {
        this.setState({
  ...
  //update score from from database
  handleScore() {
    axios
      .get("/player")
      .then(res => {
    ...
    if (player === this.state.player1) {
      axios
        ...
        .then(res => {
          if (
            res.data.length <= 0 ||
            (res.data[0].Username === "Blue" && res.data[1] === undefined)
          ) {
            this.setState({
              wins1: 1
            });
      ...
      axios
        ...
        .then(res => {
          if (
            res.data.length <= 0 ||
            (res.data[0].Username === "Red" && res.data[1] === undefined)
          ) {
            this.setState({
              wins2: 1
            });
  ...
  //check the board based on the functions from boardCheck.js
  checkBoard(board) {
    return (
    ...
    if (this.handleLoading()) {
      return (
        <div className={styles.lobby}>
          <div />
          <h1 className={styles.lobbyText}>Waiting for other players...</h1>
      ...
      return (
        <div className={styles.disconnect}>
          ...
          <h1 className={styles.errorText}>
            Error Player disconnected, please exit the page and restart browser
          </h1>
        </div>
      );
    } else {
        ...
        <div>
          <div className={styles.header}>
            <h1 className={styles.text}>
              C<span className={styles.extra}>o</span>nnect F
              <span className={styles.extra}>o</span>ur!
            </h1>
          </div>
          <button
            onClick={() => {
            ...
            <table>
              <tbody>
                {this.state.board.map((row, i) => (
                  <Board row={row} key={i} play={this.playGame} />
                ))}
              </tbody>
            </table>
            <div className={styles.score}>
              <div className={styles.scoreBox}>
                <h1 className={styles.highScore}>
                  <span className={styles.red}>Red</span> vs{" "}
                  <span className={styles.blue}>Blue</span>
                </h1>
              </div>
              <div className={styles.list}>
                {this.state.highScore.map((score, i) => (
                  <Score score={score} key={i} />
                ))}
              </div>
              <div className={styles.winBox}>{this.state.message}</div>
            </div>
          </div>
        </div>
      );
